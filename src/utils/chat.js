...
function _dedupeSaveButtons(html) {
    const saveButtons = html.find('.card-buttons [data-action=rollSave], .card-activities [data-action=rollSave]');
    if (saveButtons.length <= 1) return;

    const seen = new Set();
    saveButtons.each((i, el) => {
